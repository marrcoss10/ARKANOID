...
public class Game {
	...
	public static List<Brick> initializeBricks(List<Brick> bricks) {
		bricks.clear();
		int cont = 0;
		Double haySuerte = Math.random();
		Double ladrilloSuerte;
		if(Config.BLOCKS_X == 10){
			if (haySuerte >= 0.30){
				ladrilloSuerte = Math.random() * 30;
			}else {
				ladrilloSuerte = -1.0;
			}
			for (int iX = 0; iX < Config.BLOCKS_X; ++iX) {
				for (int iY = 0; iY < Config.BLOCKS_Y; ++iY) {
					Brick b = (new Brick(
							(iX + 1.5) * (Config.BLOCK_WIDTH + 3) + 22,
							(iY + 3) * (Config.BLOCK_HEIGHT + 3) + 50)
					);
					if ( (int) Math.round(ladrilloSuerte) == cont){
						b.setSuerte();
						cont = 100;
					}
					bricks.add(b);
			...
			for (int iX = 0; iX < Config.BLOCKS_X; ++iX) {
				for (int iY = 0; iY < Config.BLOCKS_Y; ++iY) {
					Brick b = (new Brick(
							(iX + 0.5) * (Config.BLOCK_WIDTH + 3) + 22,
							(iY + 2) * (Config.BLOCK_HEIGHT + 3) + 50)
					);
					if ( (int) Math.round(ladrilloSuerte) == cont){
						b.setSuerte();
